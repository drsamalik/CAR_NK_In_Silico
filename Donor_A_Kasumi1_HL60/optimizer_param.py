...
def Residue_Fit(x0,model_sys,data, optimizer='least_squares',fit=True):
    Sys_CAR_Gen4, Sys_CAR_Gen2, Sys_WT_NK = model_sys
    Gen4_data, Gen2_data, WT_data,ET_ratio = data
    #---Gen4--------------------#
    y0_Gen4 = [elem for i, elem in enumerate(x0) if i != 1]
    yM_NK_Gen4 = Sys_CAR_Gen4.Effector_vs_Lysis(y0_Gen4,ET_ratio)
    yD_NK_Gen4 = (Gen4_data - yM_NK_Gen4)
    #---Gen2--------------------#
    y0_Gen2 = [elem for i, elem in enumerate(x0) if i != 0]
    yM_NK_Gen2 = Sys_CAR_Gen2.Effector_vs_Lysis(y0_Gen2,ET_ratio)
    yD_NK_Gen2 = (Gen2_data - yM_NK_Gen2)
    #---WT--------------------#
    yM_WT = Sys_WT_NK.Effector_vs_Lysis(y0_Gen2,ET_ratio)
    yD_WT = (WT_data - yM_WT)
    ...
    if not fit:
        ...
        if optimizer == 'least_squares':
            return 2.0*np.concatenate((yD_NK_Gen4.astype(np.float64),yD_NK_Gen2.astype(np.float64), yD_WT.astype(np.float64)))
        else:
            return cost
...
def Residue_Pred(x0,model_sys,data,fit=True,only_wt=True):
    Sys_CAR_Gen4, Sys_CAR_Gen2, Sys_WT_NK = model_sys
    Gen4_data, Gen2_data, WT_data,ET_ratio,y0 = data
    y0[2],y0[5],y0[-1] = x0[0],x0[1],x0[2]
    ...
    if not only_wt:
        print('Ftting all')
        redsidue = 2.0*np.concatenate((yD_NK_Gen4.astype(np.float64),yD_NK_Gen2.astype(np.float64), yD_WT.astype(np.float64)))
    return redsidue