...
data_path = Path.cwd().parents[2]/'data/Kasumi1_Monocyte_CD33'
...
def prob_density_R(df, MFI_range):
    equation = df.apply(lambda col: col.dropna()[col.dropna().astype(str).str.contains('regression: y |equation|positivity threshold', case=False)] if col.dtype == 'object' else pd.Series(), axis=0).stack().values
