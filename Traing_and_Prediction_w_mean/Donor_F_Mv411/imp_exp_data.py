...
data_path = Path.cwd().parent.parent / 'data/Mv411_CD33'
def init_data(df):
...
def new_data(x):
    ...
    for i in range(len(x)):
        ...
        mean = np.mean(lysis_data)
        sigma = np.std(lysis_data)
...
def R_L_data(NK_cell, Tumor_cell,limts,CAR_Gen=None):
    ...
    #Plots(data_R_type,cell_type = NK_cell,CAR_Gen=CAR_Gen)
...
def coefficients(equation):
    ...
    return coefficients
def Plots(cell,cell_type = None,CAR_Gen=None):
    n_cols = len(cell)
    plt.rcParams.update({
    'xtick.labelsize': 20,
    'ytick.labelsize': 20,
    ...
    plt.savefig(f'{cell_type}_{CAR_Gen}.png', dpi=300, bbox_inches='tight')
    plt.show()
