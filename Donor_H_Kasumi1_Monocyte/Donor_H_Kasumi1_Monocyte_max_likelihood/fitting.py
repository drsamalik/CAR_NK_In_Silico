...
def Model_fit(x0,LB,UB,sys,data,optimizer=None,fit=True):
    if fit:
        opt_x0 = run_optimizer(x0,LB,UB,sys,data,optimizer)
        x0 = opt_x0
    yM_NK_CAR, yM_WT, x0, cost = Residue_Fit(x0,sys,data,fit=False)
    return (yM_NK_CAR, yM_WT, x0, cost)

def Model_Pred(x0,LB,UB,sys,data,fit=True,only_wt=True):
    if fit:
        result = least_squares(Residue_Pred, x0, args=(sys,data,fit,only_wt), bounds=(LB,UB))
        x0 = (result.x).tolist()
    yM_NK_CAR, yM_WT, y0, cost = Residue_Pred(x0,sys,data,fit=False)
    return (yM_NK_CAR, yM_WT, y0, cost)

def Model_bw_pred(x0,sys,data):
    Sys_CAR_75,Sys_CAR_50 = sys
