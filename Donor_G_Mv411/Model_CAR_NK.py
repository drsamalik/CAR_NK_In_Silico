...
class Model_obj_CAR_NK:
    ...
    def Cell_type_R_L(self, NK_cell, Tumor_cell, limts,CAR_Gen=None):
        R,L = R_L_Expression(NK_cell, Tumor_cell, limts,CAR_Gen=CAR_Gen)
    ...
    def Sol_Lysis(self,tD,y0,y_NK_Cell):
        ...
        tspan = [0,100]
        teval = [0,tD]
        sol = solve_ivp(odefcnCONS_LD, tspan, y0, method = 'RK45', t_eval = teval, dense_output = True, events = None, **options, args = (y_NK_Cell, self.Lam, len(y0), len(y_NK_Cell),self.r))
        yF = (1-(sol.y[:, 1].sum()/sol.y[:, 0].sum()))*100
        return yF 
    
    def Effector_vs_Lysis(self,x0,ET_ratio,tD):
        Target = 46500
...
def odefcnCONS_LD(tR, T_L0, E_R0, Lam, lenL, lenR,r):        
    dTL_dt = np.zeros(lenL)
    for Li in range(lenL):
        Lam_E_R = np.zeros(lenR)
        for Ri in range(lenR):
            Lam_E_R[Ri] = Lam[Li,Ri] * E_R0[Ri]
        dTL_dt[Li] = -np.sum(Lam_E_R)*T_L0[Li]+r*T_L0[Li]
    return dTL_dt

def R_L_Expression(NK_cell, Tumor_cell, limts,CAR_Gen=None):
    # Receptor Lygand Interaction
    data_R_type, data_L_type = R_L_data(NK_cell, Tumor_cell, limts,CAR_Gen=CAR_Gen)
